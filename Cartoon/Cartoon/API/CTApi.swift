...
let timeoutClosure = {(endpoint: Endpoint, closure: MoyaProvider<CTApi>.RequestResultClosure) -> Void in
...
let ApiProvider = MoyaProvider<CTApi>(requestClosure: timeoutClosure)
let ApiLoadingProvider = MoyaProvider<CTApi>(requestClosure: timeoutClosure, plugins: [LoadingPlugin])
...
extension MoyaProvider {
    ...
    open func request<T: HandyJSON>(_ target: Target,
        ...
        return request(target, completion: { (result) in
            ...
            guard let returnData = try? result.value?.mapModel(CTResponseData<T>.self) else {
                completion(nil)
                return
            }
