...
class CTCateController: CTBaseController {
    ...
    private lazy var searchButon: UIButton = {
           ...
           searchButon.setTitleColor(.gray, for: .normal)
    ...
    private func setupLoadData() {
        ApiLoadingProvider.request(CTApi.cateList, model: CTCateListModel.self) { (returnData) in
...
extension CTCateController: UICollectionViewDelegateFlowLayout, UICollectionViewDataSource{
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if indexPath.section == 0 {
            ...
            for tab in model.extra?.tabList ?? [] {
                ...
                vcs.append(CTComicListController(argCon: tab.argCon,
                                                    argName: tab.argName,
                                                    argValue: tab.argValue))
            }
            let vc = CTPageController(titles: titles, vcs: vcs, pageStyle: .topTabBar)
