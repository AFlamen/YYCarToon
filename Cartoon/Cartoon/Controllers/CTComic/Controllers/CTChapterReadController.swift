...
class CTChapterReadController: CTBaseController {
   ...
   private var isBarHidden: Bool = false {
       ...
       private var chapterList = [CTChapterModel]()
       ...
       private lazy var collectionView: UICollectionView = {
           ...
           collectionView.register(cellType: CTReadCollectionViewCell.self)
       ...
       lazy var topBar: CTReadTopBarView = {
           let topBar = CTReadTopBarView()
           topBar.backgroundColor = UIColor.white
           topBar.backButton.addTarget(self, action: #selector(pressBack), for: .touchUpInside)
           return topBar
       }()
       /*
       lazy var bottomBar: CTReadBottomBarView = {
           let bottomBar = CTReadBottomBarView()
       ...
       */
       convenience init(detailStatic: CTDetailStaticModel?, selectIndex: Int) {
           self.init()
       ...
       func loadData(with index: Int, isPreious: Bool, needClear: Bool, finished: ((_ finished: Bool) -> Void)? = nil) {
           ...
           if index <= -1 {
               ...
               ApiLoadingProvider.request(CTApi.chapter(chapter_id: chapterId), model: CTChapterModel.self) { (returnData) in
...
extension CTChapterReadController:UICollectionViewDelegateFlowLayout, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(for: indexPath, cellType: CTReadCollectionViewCell.self)
